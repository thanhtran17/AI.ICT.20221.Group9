...
def openFile():
    ...
    def region_of_interest(image):
        ...
        width = image.shape[1]
        polygon = np.array(
            [[(width, height), (int(width / 2), int(height / 2)), (0, height)]])
    ...
    def draw_lines(image, lines):
        ...
        if lines is not None:
            for line in lines:
                ...
                cv2.line(image_line, (int(x1), int(y1)),
                         (int(x2), int(y2)), (255, 0, 0), 10)
    ...
    lines = cv2.HoughLinesP(image_mask, 2, np.pi / 180,
                            50, np.array([]), minLineLength=40, maxLineGap=5)
    ...
    def draw_lane(image, lines):
        ...
        points_list = np.array(
            [points_list[0], points_list[1], points_list[3], points_list[2]])
...
frame = Frame(root, bg="white", width=40, height=10)
frame.pack()

label_font = font.Font(family='Helvitica', size=20)
label = Label(frame, text="Lane Detector",
              font=label_font, width=30, height=5, bg="white")
label.pack()

button_font = font.Font(family='Helvitica', size=14)
button = Button(frame, text="Open Image", command=openFile, bg='#45b592',
                fg='#ffffff',
                bd=0,
                font=button_font,
                height=1,
